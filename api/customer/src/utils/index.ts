...
export const CreateChannel = async () => {
  try {
    ...
    await channel.assertExchange(config.EXCHANGE_NAME as string, "direct", { durable: true });
    await channel.assertQueue(config.EXCHANGE_NAME as string, { durable: true });
    return channel;
  } catch (err) {
    throw err;
  }
...
export const SubscribeMessage = async (
  ...
  await channel.bindQueue(q.queue, config.EXCHANGE_NAME as string, config.CUSTOMER_SERVICE as string);
