    ...
    form.trigger(['password', 'confirmPassword']).then(async (value) => {
      if (value) {
        await createCustomer({
          email: form.getValues('email').replaceAll(' ', ''),
          password: form.getValues('password').replaceAll(' ', ''),
        })
          .unwrap()
          .then((res) => {
  ...
  async function nextButton() {
    ...
    if (select == 0) {
      form.trigger('email').then(async (value) => {
        if (value) {
          await sendEmail({ email: form.getValues('email').replaceAll(' ', '') })
            .unwrap()
            .then((res) => {
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {select != 2 && (
              <Button onClick={nextButton} className="w-full text-white">
                Next
              </Button>
            )}
          </form>
